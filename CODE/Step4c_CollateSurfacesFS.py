...
L = [fr"{basedir}\{s}\surf\{s}.L.thickness.fsLR32.func.gii" for s in subs]
R = [fr"{basedir}\{s}\surf\{s}.R.thickness.fsLR32.func.gii" for s in subs]
# concatenate and stack to array

Y = np.vstack([
    np.concatenate([
        (print(lh) or print(rh) or read_func(lh)), 
        read_func(rh)
    ])
    for lh, rh in zip(L, R)
])

out = f"Y_thickness_fslr32k_{analysis}.npy"
np.save(out, Y)
